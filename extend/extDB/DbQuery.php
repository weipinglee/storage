...
class DbQuery
{
	...
	public function __construct($name)
	{
		...
		$this->table = $name;
		$this->db = DbFactory::getInstance('thinkDB');
	}
    /**
     * @brief 给表添加表前缀
    ...
    {
        $whereObj = new \extDB\DB\WhereHandle();
        $res = $whereObj->handle($str);
        if(!empty($res)){
            if(isset($res[0])&& $res[0]!=''){
                $this->sql['where']= 'where '.preg_replace('/from\s+(\S+)/i',"from {$this->tablePre}$1 ",$res[0]);
                if(isset($res[1]) && !empty($res[1])){
                    $this->sql['bind'] = array_merge($this->sql['bind'],$res[1]);
                }
            }
        }
	...
	public function __set($name,$value)
	{
		switch($name)
		{
			...
			break;
			case 'bind':$this->sql['bind'] = array_merge($this->sql['bind'],$value);
			break;
		}
	}
    /**
    ...
    {
        return $this->paging->getPageData($url,$attrs);
    }
